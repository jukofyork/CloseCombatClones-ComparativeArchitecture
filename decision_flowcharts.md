...
These flowcharts guide architects through the critical decisions explored in this book. Each flowchart maps decision criteria to specific recommendations, with references to relevant chapters for deeper analysis.

---

## Flowchart 1: Choosing Your State Management System

**Key Question:** How should you represent what units are doing at any given moment?
...
- **YES if:** Competitive multiplayer, replay recording, or synchronized co-op
- **NO if:** Single-player only or asynchronous multiplayer
- **Rationale:** Determinism requires type-safe, reproducible state representation. String-based states risk platform-dependent hashing.

**Q2: Maximum Moddability Required?**
- **YES if:** Community-driven content, runtime unit creation, or designer-authored behaviors
- **NO if:** Curated content only or fixed mechanics
- **Rationale:** String-based states allow modders to add new states without recompilation. Type-safe enums require code changes.

**Q3: Automatic Prerequisite Chaining Required?**
- **YES if:** Complex action sequences (stand up before running, reload before firing)
- **NO if:** Simple actions or player handles all transitions manually
- **Rationale:** Bitfield prerequisite chains enable intelligent automation. Hierarchical states alone don't capture capability composition.
...
**Key Question:** How do you structure the relationship between soldiers, equipment, and squads?
...
- Use State Machines when units follow explicit orders without interpretation
...
- Use GOAP when units must plan multi-step strategies
...
**Reactive Behaviors**
- Use Behavior Trees when units must respond to changing threats
...
- Use Hybrid when non-programmers need to create AI
- Best for: Community modding, frequent iteration, complex scenarios
- Combines: Behavior trees for structure + scripts for custom logic
...
**Key Question:** What multiplayer architecture fits your game type?
...
**Real-Time Co-op**
- **Use:** Depends on acceptable latency and cheat tolerance
...
*These flowcharts synthesize nearly two decades of Close Combat clone development. Use them as starting points, but remember: every project is unique. Adapt these recommendations to your specific constraints and goals.*
...
**Part of:** Close Combat Clones Comparative Architecture Book
