...
> **Cross-References:** This appendix complements [Chapter 3: State Management](chapter_03_state_management.md), [Chapter 4: World & Terrain](chapter_04_world_terrain.md), [Chapter 11: Universal Patterns](chapter_11_universal_patterns.md), [Chapter 12: Implementation Guide](chapter_12_implementation_guide.md), [Appendix B: Unit Attribute Systems](appendix_b_unit_attribute_systems.md), and [Appendix C: File Formats](appendix_c_file_formats.md).
...
The bitfield uses the following abbreviations for states:

| Abbreviation | Full Term           | Description                          |
|--------------|---------------------|--------------------------------------|
| Sta          | Standing            | Upright posture                      |
| Pro          | Prone               | Lying down                           |
| Sto          | Stopped             | Not moving                           |
| Mov          | Moving              | Generic movement                     |
| Wal          | Walking             | Normal movement speed                |
| Run          | Running             | Fast movement                        |
| Cra          | Crawling            | Moving while prone                   |
| Fir          | Firing              | Actively shooting                    |
| Rld          | Reloading           | Weapon reload in progress            |
| WeS          | Weapon Safe         | Weapon on safety                     |
| OOA          | Out of Ammo         | No ammunition remaining              |
| NoT          | No Targets          | No valid targets available           |
| FCov         | Finding Cover       | Actively seeking protection          |
| Fol          | Following           | Moving with another unit             |
| FIF          | Firing in Formation | Synchronized group firing            |
| Def          | Defending           | Holding position                     |
| Amb          | Ambushing           | Waiting in ambush                    |
| Wait         | Waiting             | Awaiting further orders              |
| DyB          | Dying (Back)        | Dying animation (falling backward)   |
| DyF          | Dying Forward       | Dying animation (falling forward)    |
| D            | Dead                | Unit is deceased                     |

```mermaid
flowchart TD
    subgraph Bitfield["64-bit State Bitfield Layout"]
        direction LR
        bit0["Bit 0: Standing<br>(Sta)"]
...
### A.2.2 Three-Tier State Hierarchy

OpenCombat-SDL organizes states into three conceptual tiers, though stored as a flat bitfield.
...
flowchart TB
    subgraph SDL["OpenCombat-SDL State Layers"]
        ...
        C["Combat States<br>Firing, Reloading, OutOfAmmo"]
...
|----------|----------|-----------|--------------|-------------------------------------|
| Standing | Stopped  | Reloading | Defending    | Soldier on guard, ready to fire     |
| Prone    | Crawling | Reloading | -            | Soldier crawling forward cautiously |
...
-----------------------------------------------------------------------------------------------------------------
StandingFire        Combat  400     Standing Reloading              Firing                  Reloading
ProneFire           Combat  600     Prone Reloading                 Firing                  Reloading
...
class CcfQmlBaseMap:
    ...
    hypsometricImage: QString   // Path to height map
    propOpacity: float
...
### A.4.4 Cartesian Coordinate System with Hypsometric Elevation
...
class CcfQmlBaseMap:
    ...
    mHypsometricImage: Image  // Height map (RGB sum = elevation)

    function PixelInfo(x: float, y: float): int:
        // Scale coordinates to hypsometric image size
        scaleFactorX = mBackgroundImage.width / mHypsometricImage.width
        scaleFactorY = mBackgroundImage.height / mHypsometricImage.height

        pixel = mHypsometricImage.GetPixel(
            x / scaleFactorX,
            y / scaleFactorY
        )
...
| Hypsometric | Pixels | Scaled for height lookup      |
...
Map {
    ...
    hypsometricImage: "img/maps/hypsometric_test1.png"
...
By following these five guidelines, developers can create state systems that separate concerns appropriately, enable deterministic replay, and support both rapid iteration and long-term maintainability. The key is treating state as data that flows through the system rather than as mutable properties scattered throughout the codebase.

### A.6.3 World System Recommendations

The following recommendations address terrain and world system design, focusing on balancing performance, realism, and ease of content creation.
...
These world system recommendations emphasize the importance of data-driven design and performance optimization. By using established formats like TMX and PNG height maps, projects can leverage existing tools and workflows while maintaining the flexibility needed for tactical wargame simulation. The combination of tile-based grids for performance and opacity-based LOS for realism provides an optimal balance for Close Combat-style gameplay.

### A.6.4 Recommended Technology Stack

Based on the comprehensive analysis of the three Close Combat clones, the following technology stack represents a balanced approach that synthesizes the strengths of each implementation while addressing their weaknesses.
...
This technology stack provides a solid foundation for new Close Combat clone projects, offering type safety through Rust or modern C++, deterministic networking through event sourcing, and modder-friendly content pipelines. The hybrid state system combines the best aspects of all three implementations analyzed in this appendix, while the choice of established formats and tools minimizes development friction and maximizes community accessibility.
